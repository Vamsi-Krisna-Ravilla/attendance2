...
ACADEMIC_STRUCTURE = {
    "CSE": {
        ...
        "1-2": {
            ...
            "labs": {
                "CSL102": "Data Structures Lab",
                "CHL101": "Chemistry Lab"
            }
        }
    },
    "ECE": {
...
def mark_attendance_page():
    """Enhanced mobile-friendly attendance marking page with lesson plan"""
    ...
    if section and subject and period:
        ...
        if df_students is not None:
            ...
            # Add lesson plan input before submit button
...
def show_student_feedback_form():
    """Display feedback form for students"""
    # Initialize feedback sheet if needed
    initialize_feedback_sheet()
    
    # Add form header with styling
    st.markdown("""
        <div style="background: linear-gradient(135deg, #6B46C1 0%, #805AD5 100%);
                padding: 1.5rem;
                border-radius: 10px;
                ...
                color: white;
                text-align: center;">
            <h2 style="margin: 0;">RVIT - Student Feedback Form</h2>
            <p style="margin: 0.5rem 0 0 0;">Your feedback helps us improve!</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("student_feedback_form", clear_on_submit=True):
        ...
        with col1:
            ...
            if branch:
                year_sem = st.selectbox(
                    "Year and Semester *",
                    options=[''] + list(ACADEMIC_STRUCTURE[branch].keys())
                )
            else:
                year_sem = None
        ...
        # Show subject feedback only if branch and year_sem are selected
        if branch and year_sem:
            # Subject Feedback
            st.subheader("Subject Feedback")
            subject_ratings = {}
            
            if branch in ACADEMIC_STRUCTURE and year_sem in ACADEMIC_STRUCTURE[branch]:
                for subject_code, subject_name in ACADEMIC_STRUCTURE[branch][year_sem]["subjects"].items():
                    rating = st.radio(
                        f"{subject_name} *",
                ...
                lab_feedback = {}
                for lab_code, lab_name in ACADEMIC_STRUCTURE[branch][year_sem]["labs"].items():
                    st.write(f"**{lab_name}**")
                    
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        explained = st.checkbox(f"Experiments explained", key=f"exp_{lab_code}")
                    with col2:
                        executed = st.checkbox(f"Experiments executed", key=f"exe_{lab_code}")
                    with col3:
                        records = st.checkbox(f"Records corrected", key=f"rec_{lab_code}")
                ...
                if st.form_submit_button("Submit Feedback", use_container_width=True):
                    if not all([branch, year_sem, hall_ticket, student_name]):
                        st.error("Please fill all required fields")
                        return
                    
                    # Handle form submission
                    submit_feedback(
                        branch, year_sem, hall_ticket, student_name,
                        subject_ratings, lab_feedback, overall_rating, suggestions
                    )
        else:
            st.info("Please select Branch and Year/Semester to provide feedback")

def submit_feedback(branch, year_sem, hall_ticket, student_name, subject_ratings, 
                   lab_feedback, overall_rating, suggestions):
    """Handle feedback form submission"""
    try:
        # Check for existing feedback
        existing_feedback = get_feedback_data(hall_ticket)
        ...
        # Save to Excel
        df = pd.read_excel('attendance.xlsx', sheet_name='Feedback')
        ...
        # Clear session state
        st.session_state.show_student_feedback = False
...
def faculty_page():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Select", 
            ["Student Feedback", "Mark Attendance", "View Statistics", "Student Reports", 
             "Subject Analysis", "My Workload"],
            key="faculty_nav"
        )

    if page == "Student Feedback":
        st.subheader("Student Feedback Form")
        show_student_feedback_form()  # Using the same form but with different key
    
    # Rest of your faculty_page code...
    
        
    elif page == "Mark Attendance":
        # Show selection fields directly in main content area
        ...
        with col3:
            # Subject selection - only show if section is selected
            if st.session_state.sections and st.session_state.sections[0]:
                all_subjects = []
        ...
        if not (st.session_state.get('period') and st.session_state.get('sections') 
                and st.session_state.get('sections')[0] and st.session_state.get('subject')):
            st.info("Please select all class details above")
            return
    ...
    # Logout button remains in sidebar
    with st.sidebar:
...
def show_faculty_workload_admin():
    """Display faculty workload overview for admin with integrated lesson plans"""
...
def admin_page():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Select",
            ["Data Management", "Faculty Workload", "Feedback Analysis"] 
        )
    ...
    if page == "Data Management":
        try:
            with st.sidebar:
                ...
                sheet = st.selectbox(
                    "Select Sheet", 
                    [s for s in pd.ExcelFile('attendance.xlsx').sheet_names 
                     if s != 'Feedback']  # Exclude Feedback sheet from regular data management
                )
        ...
        try:
            ...
            # Convert timestamps and apply date filter
            df['Timestamp'] = pd.to_datetime(df['Timestamp'])
            mask = (df['Timestamp'].dt.date >= from_date) & (df['Timestamp'].dt.date <= to_date)
            filtered_df = df[mask]
            
            if filtered_df.empty:
                st.warning("No feedback data for selected date range")
                return
            ...
            with col3:
                unique_branches = len(filtered_df['Branch'].unique())
            ...
            for _, row in filtered_df.iterrows():
                ratings = json.loads(row['Subject_Ratings'])
            ...
            # Plot subject ratings
            fig = px.box(subject_df, x='Subject', y='Rating', color='Branch',
                        title="Subject Ratings Distribution")
            st.plotly_chart(fig, use_container_width=True)
            
            # Lab feedback analysis
            st.write("### Lab Feedback Analysis")
            lab_feedback = []
            for _, row in filtered_df.iterrows():
                labs = json.loads(row['Lab_Feedback'])
            ...
            lab_df = pd.DataFrame(lab_feedback)
            
            # Calculate lab completion percentages
            lab_stats = lab_df.groupby('Lab').agg({
                'Explained': 'mean',
                'Executed': 'mean',
                'Records': 'mean'
            }).round(3) * 100
            ...
            st.dataframe(
                recent[['Timestamp', 'Branch', 'Student_Name', 'Overall_Rating', 'Suggestions']],
                use_container_width=True
            )
            ...
            if st.button("Export Analysis"):
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    filtered_df.to_excel(writer, sheet_name='All Feedback', index=False)
                    subject_df.to_excel(writer, sheet_name='Subject Analysis', index=False)
                    lab_stats.to_excel(writer, sheet_name='Lab Analysis')
                
                st.download_button(
                    label="📥 Download Analysis Report",
                    data=buffer.getvalue(),
                    file_name=f"feedback_analysis_{datetime.now().strftime('%Y%m%d')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
...
def initialize_feedback_sheet():
    """Initialize feedback sheet if it doesn't exist"""
    try:
        ...
        try:
            ...
            df = pd.DataFrame(columns=[
                'Timestamp', 'Branch', 'Year_Semester', 'Hall_Ticket', 
                'Student_Name', 'Subject_Ratings', 'Lab_Feedback',
                'Overall_Rating', 'Suggestions'
            ])
            df.to_excel('attendance.xlsx', sheet_name='Feedback', index=False)
            return
            
        if 'Feedback' not in xls.sheet_names:
            df = pd.DataFrame(columns=[
                'Timestamp', 'Branch', 'Year_Semester', 'Hall_Ticket', 
                'Student_Name', 'Subject_Ratings', 'Lab_Feedback',
                'Overall_Rating', 'Suggestions'
            ...
            with pd.ExcelWriter('attendance.xlsx', mode='a', engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='Feedback', index=False)
        ...
        st.error(f"Error initializing feedback sheet: {str(e)}")

def get_feedback_data(hall_ticket):
    """Check if student has already submitted feedback"""
    try:
        df = pd.read_excel('attendance.xlsx', sheet_name='Feedback')
        return df[df['Hall_Ticket'] == hall_ticket]
    except Exception:
        return pd.DataFrame()

def show_feedback_form(form_key="student_feedback_form"):
    """Display the student feedback form"""
    # Initialize feedback sheet if needed
    initialize_feedback_sheet()
        ...
        if branch and year_sem:
            ...
            if submitted:
                ...
                try:
                    ...
                    # Read existing feedback or create new DataFrame
                    try:
                        df = pd.read_excel('attendance.xlsx', sheet_name='Feedback')
                    except:
                        df = pd.DataFrame()
                    ...
                    # Success message and animations
                    st.success("Thank you! Your feedback has been submitted successfully.")
                    st.balloons()
...
def mark_attendance(section, period, attendance_data, username, subject, lesson_plan):
    ...
    try:
        ...
        if success:
            update_faculty_log(faculty_name, section, period, subject, lesson_plan)
...
def update_faculty_log(faculty_name, section, period, subject, lesson_plan):
    """Update faculty attendance log including lesson plan"""
    try:
        # Read faculty sheet
        ...
        # Create log entry in the format: DD/MM/YYYY_HH:MM[AM/PM]_[Period]_[Subject]_[Section]_[LessonPlan]
        log_entry = f"{current_date.strftime('%d/%m/%Y_%I:%M%p')}_{period}_{subject}_{section}_{lesson_plan}"
...
def show_student_feedback():
    """Display student feedback page with enhanced UI"""
    ...
    st.markdown("""
        <div style="background: linear-gradient(135deg, #6B46C1 0%, #805AD5 100%);
                  ...
                  margin-bottom: 2rem;
                  text-align: center;">
            <h2 style="margin: 0;">Student Feedback Form</h2>
            <p style="margin-top: 0.5rem; opacity: 0.9;">Help us improve your learning experience!</p>
        </div>
    """, unsafe_allow_html=True)
        ...
        if branch and year_sem:
            ...
            if branch in ACADEMIC_STRUCTURE and year_sem in ACADEMIC_STRUCTURE[branch]:
                ...
                suggestions = st.text_area(
                    ...
                    placeholder="Please share your thoughts on how we can improve your learning experience..."
                )
                ...
                if st.form_submit_button("Submit Feedback", use_container_width=True):
                    ...
                    existing_feedback = get_feedback_data(hall_ticket)
                    if not existing_feedback.empty:
                        st.error("You have already submitted feedback for this semester")
                        return
                    ...
                    try:
                        ...
                        # Save feedback
                        df = pd.read_excel('attendance.xlsx', sheet_name='Feedback') if os.path.exists('attendance.xlsx') else pd.DataFrame()
                        df = pd.concat([df, pd.DataFrame([feedback_data])], ignore_index=True)
            ...
            st.info("Please select your Branch and Year/Semester to proceed")


def main():
    st.title("RVIT - Academic Portal")
    
    # Main tabs
    tab1, tab2 = st.tabs(["Student", "Faculty/Admin"])
    
    with tab1:
        st.subheader("Student Portal")
        
        # Create buttons for different student options
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("📝 Feedback Form", use_container_width=True, key="student_feedback_btn"):
                st.session_state.show_student_feedback = True  # Changed key name
        with col2:
            marks_selected = st.button("📊 View Marks", use_container_width=True, disabled=True)
        with col3:
            other_selected = st.button("📚 Other Services", use_container_width=True, disabled=True)
            
        # Show feedback form when selected
        if 'show_student_feedback' in st.session_state and st.session_state.show_student_feedback:
            show_student_feedback_form()  # New function for student feedback
    ...
    with tab2:
        ...
        with st.container():
            ...
            # Create columns for username/password
            col1, col2 = st.columns(2)
            with col1:
            ...
            with st.expander("👤 Credential Management"):
                tab_reset1, tab_reset2 = st.tabs(["Reset Password", "Reset Username"])
        ...
        # Show faculty/admin content if logged in
