...
def mark_attendance(section, period, attendance_data, faculty_name, subject):
    """Mark attendance in original sections and track unsuccessful records"""
    try:
        date_str = datetime.now().strftime('%d/%m/%Y')
        time_str = datetime.now().strftime('%I:%M%p')
        
        # Track unsuccessful records
        ...
        for orig_section, students in original_sections.items():
            try:
                ...
                # Log section-level failures but don't show error message
        ...
        # Try to get names for unsuccessful records
        if unsuccessful_records:
            try:
                student_df = pd.read_excel('attendance.xlsx', sheet_name=section)
                for record in unsuccessful_records:
                    if record['Student Name'] == "Unknown":
                        record['Student Name'] = student_df[student_df['HT Number'] == record['HT Number']]['Student Name'].iloc[0]
            except:
                pass
        
        return True, unsuccessful_records
    
    except Exception as e:
        return False, unsuccessful_records
...
def get_faculty_workload(username):
    ...
    try:
        ...
        if not df_workload.empty:
            df_workload = df_workload.sort_values('Date')
            # Create month-year format for grouping
            df_workload['Month'] = df_workload['Date'].dt.strftime('%b%Y')
            # Convert date back to string format for display
            df_workload['Date'] = df_workload['Date'].dt.strftime('%d/%m/%Y')
        
        return total_periods, df_workload
    except Exception as e:
...
def workload_analysis_page():
    ...
    if not workload_df.empty:
        ...
        with col1:
            ...
            current_month_data = workload_df[workload_df['Month'] == current_month].copy()
            if not current_month_data.empty:
                current_month_data = current_month_data.drop('Month', axis=1)
                csv = current_month_data.to_csv(index=False)
                st.download_button(
                    "Download Current Month",
                    data=csv,
                    file_name=f"workload_{current_month}.csv",
                    mime="text/csv"
                )
        ...
        with col2:
            ...
            if st.button("Download Complete Report"):
                # Remove Month column
                download_df = workload_df.drop('Month', axis=1)
                csv = download_df.to_csv(index=False)
                st.download_button(
                    label="Download CSV",
                    data=csv,
                    file_name=f"workload_complete_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv"
                )
    else:
