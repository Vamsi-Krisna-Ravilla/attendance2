...
def safe_json_loads(data):
    ...
    try:
        ...
        try:
            # Try to evaluate string representation of dictionary
...
def analyze_feedback_data(df):
    ...
    try:
        ...
        # Safely convert string representations to dictionaries
        ...
        # Subject-wise analysis with error handling
        subject_ratings = []
        ...
        subject_avg = subject_df.groupby('Subject')['Rating'].mean() if not subject_df.empty else pd.Series()
        
        # Branch-wise analysis
        branch_avg = df.groupby('Branch')['Overall_Rating'].mean()
...
def show_admin_dashboard():
    ...
    try:
        ...
        with col4:
            st.metric("Total Branches", len(metrics['branch_avg']))
        ...
        try:
            ...
            if selected_branches:
                mask = mask & df['Branch'].isin(selected_branches)
            filtered_df = df[mask]
        except Exception as e:
        ...
        try:
            ...
            for _, row in filtered_df.iterrows():
                try:
                    ratings = safe_json_loads(row['Subject_Ratings'])
                    for subject, rating in ratings.items():
                        subject_ratings.append({
                            'Subject': subject,
                            'Rating': float(rating)
            ...
            if not subject_df.empty:
                fig_subjects = px.bar(
                    subject_df.groupby('Subject')['Rating'].mean().reset_index(),
                    x='Subject',
                    y='Rating',
...
def show_feedback_form():
    """Display student feedback form with enhanced error handling"""
    try:
        st.markdown("""
