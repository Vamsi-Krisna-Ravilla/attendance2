...
def get_student_data(section):
    ...
    try:
        ...
        return df.fillna('')  # Replace NaN with empty string
    except Exception as e:
...
def faculty_page():
    ...
    with st.sidebar:
        ...
        page = st.radio("Select", ["Mark Attendance", "View Statistics", "Student Reports", "Subject Analysis"])
...
def admin_page():
    ...
    with col2:
        if st.button("Download Current Sheet"):
            csv = edited_df.to_csv(index=False)
            st.download_button(
                label="Download CSV",
                data=csv,
                file_name=f"{sheet}_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
